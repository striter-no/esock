...
int esock_abs_connect(
    esock_abs *abs
){
    if (!abs || !abs->csock_connect) return -1;
    return abs->csock_connect(&abs->nfd);
}

int esock_abs_sbinds(
    esock_abs *abs,
    int  (*ssock_bind)(enet_fd *fd),
    int  (*ssock_newcli)(enet_fd server, enet_fd *newcli),
    ssize_t (*ssock_recv)(enet_fd server, im_fd *fd),
    ...
    if (!abs) return -1;
    abs->ssock_bind = ssock_bind;
    abs->ssock_newcli = ssock_newcli;
    abs->ssock_recv = ssock_recv;
    abs->ssock_send = ssock_send;
    return 0;
}

int esock_abs_cbinds(
    esock_abs *abs,
    int  (*csock_connect)(enet_fd *connected),
    ssize_t (*csock_recv)(enet_fd client, im_fd *fd),
    ssize_t (*csock_send)(enet_fd client, im_fd *fd)
){
    if (!abs) return -1;
    abs->csock_connect = csock_connect;
    abs->csock_recv = csock_recv;
    abs->csock_send = csock_send;
    return 0;
}

void esock_abs_create(
    esock_abs *abs, 
    int (*sock_create)(enet_fd *fd, eaddr_t address),
    eaddr_t address
){
    sock_create(&abs->nfd, address);
}
